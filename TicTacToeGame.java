    	...
    	// YOUR CODE HERE
			if(countTransform != 0){
				if(this.lines == this.columns){
					...
					else Utils.rotate(this.lines,this.columns,this.transformedBoard);
				}
				else{
					if(countTransform == 2)Utils.verticalFlip(this.lines,this.columns,this.transformedBoard);
					else Utils.horizontalFlip(this.lines,this.columns,this.transformedBoard);
  	...
  	public boolean equalsWithSymmetry(TicTacToeGame other){
  		...
  		// YOUr CODE HERE
			if(this.lines == other.lines && this.columns == other.columns && this.gameState == other.getGameState()){
				...
				while(other.hasNext()){
					other.next();
					boolean symFound = true;
					for(int i = 0; i < this.lines * this.columns; i++){
						if(this.board[i] != other.board[other.transformedBoard[i]]){
							i = 666;
							symFound = false;
						}
					}
					if(symFound)return true;
				}
			}
			return false;
